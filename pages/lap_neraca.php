<?php
// Laporan Neraca Actual - responsive, compact, tanpa summary card.
?>
      ...
      <div class="ln-brand-main">
        ...
        <div class="ln-brand-copy">
          <div class="ln-title-row">
            <h1>Neraca Aktual</h1>
            <button type="button" id="lnInfoBtn" class="ln-info-btn" title="Informasi neraca" aria-label="Buka informasi neraca">i</button>
          </div>
          <p>Posisi Aktiva dan Pasiva berdasarkan acc_history aktual.</p>
        </div>
      </div>
  ...
  <section class="ln-table-card">
    ...
    <div class="ln-table-toolbar">
      ...
      <div class="ln-toolbar-right">
        <div id="lnBalanceStatus" class="ln-balance-status ln-balance-neutral" title="Status keseimbangan neraca">
        ...
        <label class="ln-search" aria-label="Cari akun neraca">
    ...
    <div class="ln-mobile-tabs" role="tablist" aria-label="Sisi neraca">
      <button type="button" id="lnTabAktiva" class="ln-mobile-tab active" data-side="aktiva" onclick="setLnMobileSide('aktiva')" role="tab" aria-selected="true">
        Aktiva
        <small id="lnTabAktivaTotal">Rp 0</small>
      </button>
      <button type="button" id="lnTabPasiva" class="ln-mobile-tab" data-side="pasiva" onclick="setLnMobileSide('pasiva')" role="tab" aria-selected="false">
        Pasiva
        <small id="lnTabPasivaTotal">Rp 0</small>
      </button>
    </div>
      ...
      <table id="lnTable">
        ...
        <thead>
          <tr class="ln-group-head">
            <th colspan="3" class="ln-group-asset">Aktiva <span>(Assets)</span></th>
            <th colspan="3" class="ln-group-passive">Pasiva <span>(Liabilities &amp; Equity)</span></th>
          </tr>
        ...
        <tfoot>
          <tr>
            <td colspan="2" class="ln-foot-label ln-foot-asset">Total Aktiva</td>
            <td id="lnFootAktiva" class="ln-foot-money ln-foot-asset">Rp 0</td>
            <td colspan="2" class="ln-foot-label ln-foot-passive">Total Pasiva</td>
            <td id="lnFootPasiva" class="ln-foot-money ln-foot-passive">Rp 0</td>
          </tr>
        </tfoot>
      </table>
    </div>
    ...
    <!-- Mobile total dibuat di luar area scroll agar selalu terlihat, termasuk Safari iPhone -->
    <div id="lnMobileTotalBar" class="ln-mobile-totalbar" aria-live="polite">
      <span id="lnMobileFootLabel">Total Aktiva</span>
  ...
  <div class="ln-info-card">
    <div class="ln-info-header">
      <div class="ln-info-heading">
        ...
        <div>
          <h3 id="lnInfoTitle">Informasi Neraca Aktual</h3>
          <p>Ringkasan posisi dan cara membaca keseimbangan Aktiva dengan Pasiva.</p>
        </div>
      </div>
    ...
    <div class="ln-info-body">
      ...
      <div id="lnInfoCondition" class="ln-info-condition">
        Neraca akan menampilkan keseimbangan antara total Aktiva dan total Pasiva pada posisi yang dipilih.
      </div>

      <div class="ln-info-grid">
        <section class="ln-info-block ln-info-blue">
          <div class="ln-info-block-title"><span>A</span> Aktiva</div>
          <p>Menunjukkan sumber daya/aset yang dimiliki. Akun disusun bertingkat berdasarkan kode perkiraan agar struktur akun mudah ditelusuri.</p>
        </section>
        <section class="ln-info-block ln-info-green">
          <div class="ln-info-block-title"><span>P</span> Pasiva</div>
          <p>Menunjukkan kewajiban dan ekuitas sebagai sumber pendanaan Aktiva. Total Pasiva idealnya seimbang dengan total Aktiva.</p>
        </section>
        <section class="ln-info-block ln-info-amber">
          <div class="ln-info-block-title"><span>Δ</span> Selisih</div>
          <p>Selisih dihitung dari Total Aktiva dikurangi Total Pasiva. Nilai mendekati nol menunjukkan posisi neraca seimbang.</p>
        </section>
        <section class="ln-info-block ln-info-slate">
          <div class="ln-info-block-title"><span>⌕</span> Pencarian</div>
          <p>Pencarian memfilter kode maupun nama perkiraan. Pada mobile, gunakan tab Aktiva/Pasiva agar daftar tetap lebar dan mudah dibaca.</p>
        </section>
      </div>

      <div class="ln-info-note">
        Gunakan pilihan tanggal dan Area/Cabang untuk membandingkan posisi yang benar. Apabila terdapat selisih yang material, lakukan pengecekan mapping akun atau data sumber sebelum laporan digunakan untuk pengambilan keputusan.
      </div>
    </div>
  </div>
</div>
...
<style>
  ...
  @media (max-width:767px) {
    ...
    .ln-mobile-tab.active[data-side="aktiva"] { color:#4338ca; border-color:#c7d2fe; background:#eef2ff; }
    .ln-mobile-tab.active[data-side="pasiva"] { color:#047857; border-color:#a7f3d0; background:#ecfdf5; }
...
<script>
  ...
  let lnMobileSide = 'aktiva';
  let lnLevelMode = '5';
  const lnTreeState = {
    aktiva: { expanded:new Set(), collapsed:new Set(), meta:new Map() },
    pasiva: { expanded:new Set(), collapsed:new Set(), meta:new Map() }
  };
  ...
  async function fetchLapNeraca() {
    ...
    try {
      ...
      console.error(e);
      lnRaw = null;
      if (body) body.innerHTML = `<tr><td colspan="6" class="ln-empty">Gagal memuat data neraca.</td></tr>`;
      if (mobileBody) mobileBody.innerHTML = `<tr><td colspan="3" class="ln-empty">Gagal memuat data neraca.</td></tr>`;
  ...
  function renderSide(row, side) {
    ...
    const sideClass = side === 'pasiva' ? 'ln-side-passive' : 'ln-side-asset';
  ...
  function renderMobileRow(row, side) {
    ...
    const passiveClass = side === 'pasiva' ? 'ln-side-passive' : 'ln-side-asset';
    return `
      <tr>
  ...
  function renderLnMobileTable() {
    ...
    table.classList.toggle('ln-mobile-passive', lnMobileSide === 'pasiva');
    document.getElementById('lnMobileTotalBar')?.classList.toggle('ln-mobile-passive', lnMobileSide === 'pasiva');
    footLabel.textContent = lnMobileSide === 'pasiva' ? 'Total Pasiva' : 'Total Aktiva';
  ...
  function setLnMobileSide(side) {
    lnMobileSide = side === 'pasiva' ? 'pasiva' : 'aktiva';
    const aktivaBtn = document.getElementById('lnTabAktiva');
    const pasivaBtn = document.getElementById('lnTabPasiva');
    aktivaBtn?.classList.toggle('active', lnMobileSide === 'aktiva');
    pasivaBtn?.classList.toggle('active', lnMobileSide === 'pasiva');
    aktivaBtn?.setAttribute('aria-selected', lnMobileSide === 'aktiva' ? 'true' : 'false');
    pasivaBtn?.setAttribute('aria-selected', lnMobileSide === 'pasiva' ? 'true' : 'false');
    renderLnMobileTable();
    document.querySelector('.ln-mobile-table-wrap')?.scrollTo({top:0,left:0,behavior:'auto'});
  }

  function updateLnBalanceStatus(selisih) {
    ...
    if (selisih === null || selisih === undefined || !Number.isFinite(Number(selisih))) {
    ...
    const abs = Math.abs(n);
    const base = Math.max(Math.abs(Number(lnRaw?.totals?.aktiva || 0)), 1);
    const ratio = (abs / base) * 100;

    if (abs === 0) {
      el.classList.add('ln-balance-ok');
      el.innerHTML = `<span class="ln-status-dot"></span><span>Balance</span>`;
    } else if (ratio <= 0.01) {
      el.classList.add('ln-balance-warn');
      el.innerHTML = `<span class="ln-status-dot"></span><span>Selisih ${fmtLnShort(abs)}</span>`;
    } else {
  ...
  function renderLapNeraca() {
    ...
    const pasiva = getLnVisibleRows('pasiva', query);
    const maxRows = Math.max(aktiva.length, pasiva.length);
    const body = document.getElementById('lnBody');
    ...
    if (maxRows === 0) {
      ...
      for (let i = 0; i < maxRows; i++) {
        html += `<tr>${renderSide(aktiva[i], 'aktiva')}${renderSide(pasiva[i], 'pasiva')}</tr>`;
      }
      body.innerHTML = html;
    }

    const totalAktiva = Number(lnRaw?.totals?.aktiva || 0);
    const totalPasiva = Number(lnRaw?.totals?.pasiva || 0);
    const selisih = Number(lnRaw?.totals?.selisih ?? (totalAktiva - totalPasiva));
    const aktivaRows = (lnRaw?.aktiva || []).length;
    const pasivaRows = (lnRaw?.pasiva || []).length;

    document.getElementById('lnFootAktiva').textContent = fmtLnRp(totalAktiva);
    document.getElementById('lnFootPasiva').textContent = fmtLnRp(totalPasiva);
    document.getElementById('lnTabAktivaTotal').textContent = fmtLnShort(totalAktiva);
    document.getElementById('lnTabPasivaTotal').textContent = fmtLnShort(totalPasiva);
  ...
  function renderLnInfo() {
    ...
    const totalPasiva = Number(lnRaw?.totals?.pasiva || 0);
    const selisih = Number(lnRaw?.totals?.selisih ?? (totalAktiva - totalPasiva));
    const isBalanced = Math.abs(selisih) === 0;

    summary.innerHTML = `
      <div class="ln-info-stat">
        <div class="ln-info-stat-label">Total Aktiva</div>
        <div class="ln-info-stat-value blue">${fmtLnRp(totalAktiva)}</div>
      </div>
      <div class="ln-info-stat">
        <div class="ln-info-stat-label">Total Pasiva</div>
        <div class="ln-info-stat-value green">${fmtLnRp(totalPasiva)}</div>
      </div>
      <div class="ln-info-stat">
        <div class="ln-info-stat-label">Selisih</div>
        <div class="ln-info-stat-value ${isBalanced ? 'green' : 'red'}">${fmtLnRp(selisih)}</div>
      </div>
    `;

    if (isBalanced) {
      condition.innerHTML = `<b>Posisi neraca seimbang.</b> Total Aktiva sama dengan Total Pasiva pada ${safeLn(lnRaw?.scope_label || 'scope terpilih')} posisi ${safeLn(lnRaw?.tanggal || '-')}.`;
    } else {
      condition.innerHTML = `<b>Terdapat selisih ${fmtLnRp(selisih)}.</b> Lakukan pengecekan mapping akun/data sumber pada ${safeLn(lnRaw?.scope_label || 'scope terpilih')} sebelum laporan digunakan sebagai posisi final.`;
    }
  }
  ...
  function exportLapNeraca() {
    ...
    const aktiva = lnRaw.aktiva || [];
    const pasiva = lnRaw.pasiva || [];
    const maxRows = Math.max(aktiva.length, pasiva.length);
    let html = `<table border="1"><thead><tr><th colspan="3">AKTIVA</th><th colspan="3">PASIVA</th></tr><tr><th>Kode</th><th>Uraian</th><th>Saldo</th><th>Kode</th><th>Uraian</th><th>Saldo</th></tr></thead><tbody>`;
    for (let i = 0; i < maxRows; i++) {
      const a = aktiva[i] || {};
      const p = pasiva[i] || {};
    ...
    html += `<tr><td colspan="2"><b>Total Aktiva</b></td><td>${Math.round(Number(lnRaw?.totals?.aktiva || 0))}</td><td colspan="2"><b>Total Pasiva</b></td><td>${Math.round(Number(lnRaw?.totals?.pasiva || 0))}</td></tr></tbody></table>`;
    const blob = new Blob([html], {type:'application/vnd.ms-excel'});
    const a = document.createElement('a');
    const url = URL.createObjectURL(blob);
    a.href = url;
    a.download = `Lap_Neraca_Actual_${lnRaw?.tanggal || 'data'}.xls`;
