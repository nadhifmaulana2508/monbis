...
<div class="max-w-[1400px] mx-auto px-2 md:px-4 py-4 md:py-6 bg-gray-50 min-h-screen font-sans">
  ...
  <div id="contentDash" class="hidden space-y-4 md:space-y-6 overflow-x-hidden">
    ...
    <div class="grid lg:grid-cols-12 gap-3 md:gap-4 mt-4 md:mt-6">
      ...
      <div class="bg-white p-4 md:p-5 rounded-xl md:rounded-3xl shadow-sm border border-gray-100 lg:col-span-3 flex flex-col h-[400px] md:h-[450px]">
        ...
        <div id="box_runoff_realisasi" class="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0"></div>
      ...
      <div class="bg-white p-4 md:p-5 rounded-xl md:rounded-3xl shadow-sm border border-gray-100 lg:col-span-3 flex flex-col h-[400px] md:h-[450px]">
        ...
        <div id="box_flow_recovery" class="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0"></div>
      ...
      <div class="bg-white p-4 md:p-6 rounded-xl md:rounded-3xl shadow-sm border border-gray-100 lg:col-span-6 relative flex flex-col h-[400px] md:h-[450px]">
...
<script>
  ...
  function fetchDashboardUtama() {
    ...
    // 🚀 FIX TOTAL NAIK ATAS DAN BYPASS PENGECEKAN
    Promise.all([pRunoffKorwil, pFlowKorwil]).then(([roRaw, flowRaw]) => {
      ...
      if(ro && ro.detail_korwil) {
        // Ambil data selain total untuk menghindari duplicate jika dari API udah ada di bawah
        let runoffData = ro.detail_korwil.filter(k => !(k.nama_korwil || '').toUpperCase().includes('TOTAL'));
        
        // Paksakan grand_total ditaruh paling atas
        if(ro.grand_total) {
            let totalData = {...ro.grand_total};
            totalData.nama_korwil = totalData.nama_korwil || totalData.nama_cabang || `TOTAL KESELURUHAN`;
            runoffData.unshift(totalData); 
        }
      ...
      if(flow && flow.detail_korwil) {
        let flowData = flow.detail_korwil.filter(k => !(k.nama_korwil || '').toUpperCase().includes('TOTAL'));
        
        if(flow.grand_total) {
             let totalData = {...flow.grand_total};
             totalData.nama_korwil = totalData.nama_korwil || totalData.nama_cabang || `TOTAL KESELURUHAN`;
             flowData.unshift(totalData); 
        }
    ...
    Promise.all([pTopReal, pTopNpl, pDeltaNpl, pRrCabang]).then(([realRaw, nplRaw, deltaRaw, rrRaw]) => {
      ...
      if(real.top_cabang) {
        ...
        // FIX NAMA AO AGAR LEBIH AGRESIF MUNCUL NAMA CABANGNYA
        let topAOCustom = (real.top_ao || []).map(ao => {
            let namaPegawai = ao.nama_ao || ao.nama_pegawai || 'Unknown';
            let namaCabang = ao.nama_cabang || ao.cabang || '';
            let namaCustom = namaPegawai;
            
            if (namaCabang && namaCabang.toLowerCase() !== 'unknown') {
                let cabangShort = namaCabang.replace(/Kc\. /gi, '').trim();
                namaCustom = `${namaPegawai} (${cabangShort})`;
            }
