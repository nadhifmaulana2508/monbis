...
(()=>{
  ...
  const post=async payload=>{const response=await fetch(API,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),json=await response.json();if(!response.ok||Number(json.status)!==200)throw Error(json.message||'Perubahan gagal disimpan');return json.data||{}};
