...
?>
<script>
(()=>{const API='./api/index.php?request=kpi',EDIT_ICON=<?= json_encode(mb_svg('edit')) ?>,el=id=>document.getElementById(id),ui=()=>window.MonbisUI||{},esc=v=>ui().escape?ui().escape(v):String(v??''),num=v=>Number(v||0),fmt=v=>new Intl.NumberFormat('id-ID',{style:'percent',maximumFractionDigits:1}).format(num(v));let rows=[];
function kpiHeaders(){const match=document.cookie.match(/(?:^|;\s*)sso_token=([^;]+)/),token=String((match?decodeURIComponent(match[1]):'')||window.AUTH_TOKEN||localStorage.getItem('dpk_token')||'').replace(/^Bearer\s+/i,'').trim(),headers={'Content-Type':'application/json'};if(token)headers.Authorization='Bearer '+token;return headers}
