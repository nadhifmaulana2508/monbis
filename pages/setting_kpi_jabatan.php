...
$head=mb_build_grouped_thead([
 ['label'=>'Jabatan','class'=>'mb-sticky-left','attrs'=>['style'=>'width:120px']],['label'=>'Kelompok','attrs'=>['style'=>'width:120px']],['label'=>'Indikator','class'=>'mb-text-left','attrs'=>['style'=>'width:250px']],['label'=>'Bobot','attrs'=>['style'=>'width:82px']],['label'=>'Target Default','attrs'=>['style'=>'width:110px']],['label'=>'Arah','attrs'=>['style'=>'width:90px']],['label'=>'Unit','attrs'=>['style'=>'width:90px']],['label'=>'Status','attrs'=>['style'=>'width:100px']]
]);
...
mb_render_report_page([
 ...
 'toolbar'=>[
   ...
   'actions'=>[['attrs'=>['id'=>'settingKpiSave'],'tone'=>'success','icon'=>'save','title'=>'Simpan semua perubahan','aria_label'=>'Simpan semua perubahan']]
 ],
 'table'=>[
...
.mb-kpi-setting-page #settingKpiTable th,.mb-kpi-setting-page #settingKpiTable td{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:3px 3px;font-size:9px}
.mb-kpi-setting-page #settingKpiTable th[data-score-column],.mb-kpi-setting-page #settingKpiTable td[data-score-column]{width:53px;min-width:53px;padding-left:1px;padding-right:1px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(1),.mb-kpi-setting-page #settingKpiTable td:nth-child(1){width:72px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(2),.mb-kpi-setting-page #settingKpiTable td:nth-child(2){width:82px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(3),.mb-kpi-setting-page #settingKpiTable td:nth-child(3){width:190px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(4),.mb-kpi-setting-page #settingKpiTable td:nth-child(4){width:54px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(11),.mb-kpi-setting-page #settingKpiTable td:nth-child(11){width:100px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(12),.mb-kpi-setting-page #settingKpiTable td:nth-child(12){width:58px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(13),.mb-kpi-setting-page #settingKpiTable td:nth-child(13){width:55px}
.mb-kpi-setting-page #settingKpiTable th:nth-child(14),.mb-kpi-setting-page #settingKpiTable td:nth-child(14){width:62px}
.mb-kpi-setting-page .mb-inline-score-save,.mb-kpi-setting-page .mb-kpi-row-save{display:none!important}
...
let rows=[],scoreRows=[];
function kpiToken(){let token='';try{token=String(window.AUTH_TOKEN||localStorage.getItem('dpk_token')||'').trim()}catch(error){}if(!token){const match=document.cookie.match(/(?:^|;\s*)sso_token=([^;]+)/);if(match)try{token=decodeURIComponent(match[1])}catch(error){token=match[1]}}return token}
function kpiHeaders(extra={}){const headers=Object.assign({'Content-Type':'application/json'},extra),token=kpiToken();if(token)headers.Authorization=/^Bearer\s/i.test(token)?token:`Bearer ${token}`;return headers}
window.MonbisKpiHeaders=kpiHeaders;
...
<script>
(()=>{
  const button=document.getElementById('settingKpiSave'),body=document.getElementById('settingKpiBody');
  if(!button||!body)return;
  const API='./api/index.php?request=kpi',num=value=>Number(value||0);
  const parseWeight=value=>{let s=String(value??'').trim().replace(',','.');if(s.endsWith('%'))return (parseFloat(s.slice(0,-1))||0)/100;const n=parseFloat(s);return Number.isFinite(n)?(n>1?n/100:n):0};
  const parseTarget=(value,unit)=>{let s=String(value??'').trim().replace(/^Rp\s*/i,'').replace(/\s/g,'');if(unit==='PERSEN'&&s.endsWith('%'))return (parseFloat(s.slice(0,-1).replace(',','.'))||0)/100;s=s.replace(/\./g,'').replace(',','.');return Number(s)||0};
  const parseScore=(value,unit)=>{let s=String(value??'').trim().replace(',','.');if(!s)return 999;if(unit==='PERSEN'&&s.endsWith('%'))return (parseFloat(s.slice(0,-1))||0)/100;const n=parseFloat(s);return Number.isFinite(n)?n:0};
  const post=async payload=>{const response=await fetch(API,{method:'POST',credentials:'same-origin',headers:window.MonbisKpiHeaders(),body:JSON.stringify(payload)}),json=await response.json();if(!response.ok||Number(json.status)!==200)throw Error(json.message||'Perubahan gagal disimpan');return json.data||{}};
  const saveAll=async()=>{if(button.dataset.saving==='1')return;const indicators=[...body.querySelectorAll('tr[data-id]')].map(row=>{const weight=row.querySelector('[data-field="bobot"]'),target=row.querySelector('[data-field="target"]'),status=row.querySelector('[data-field="status"]');return {type:'save_indicator',id:Number(row.dataset.id),bobot:parseWeight(weight?.value),status:status?.value||'AKTIF',target:parseTarget(target?.value,row.dataset.unit||'')};}),scores=[...body.querySelectorAll('.mb-inline-score')].map(box=>{const min=box.querySelector('[data-score-field="min_indeks"]')?.value,max=box.querySelector('[data-score-field="max_indeks"]')?.value,unit=box.dataset.scoreUnit||'';return {type:'save_score',id:Number(box.dataset.scoreId),min_indeks:parseScore(min,unit),max_indeks:parseScore(max,unit)}});if(scores.some(item=>item.max_indeks<item.min_indeks)){alert('Indeks maksimum tidak boleh lebih kecil dari minimum.');return}button.dataset.saving='1';button.disabled=true;try{const scoreData=await post({type:'setting_scores'}),predicates=new Map((scoreData.parameter_skor||[]).map(item=>[Number(item.id),item.predikat||'']));await Promise.all(indicators.map(item=>post(item)));await Promise.all(scores.map(item=>post({...item,predikat:predicates.get(item.id)||'',aktif:1})));alert('Semua perubahan KPI berhasil disimpan.');window.location.reload()}catch(error){alert(error.message)}finally{button.disabled=false;delete button.dataset.saving}};
  button.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();saveAll()},true);
})();
</script>
<script>
(() => {
  ...
  async function loadScores(){try{const response=await fetch('./api/index.php?request=kpi',{method:'POST',credentials:'same-origin',headers:window.MonbisKpiHeaders(),body:JSON.stringify({type:'setting_scores'})});const json=await response.json();scoreRows=json.data?.parameter_skor||[];decorate()}catch(error){console.error('Gagal memuat range skor',error)}}
