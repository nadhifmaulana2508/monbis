...
(() => {
  ...
  async function loadScores(){try{const response=await fetch('./api/index.php?request=kpi',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'setting'})});const json=await response.json();scoreRows=json.data?.parameter_skor||[];decorate()}catch(error){console.error('Gagal memuat range skor',error)}}
