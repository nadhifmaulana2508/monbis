...
<style>
  ...
  /* ========================================================
     TABEL MODAL DETAIL RR (FIX STICKY & TAMPILAN)
     ======================================================== */
  ...
  #tableExportRR thead th { position: sticky !important; top: 0 !important; z-index: 40 !important; background-color: #f1f5f9 !important; box-shadow: inset 0 -2px 0 #cbd5e1; }
  ...
  #headModalRR th.col-rek, #headModalRR th.col-nas, #headModalRR th.col-nas-lunas { z-index: 50 !important; background-color: #e2e8f0 !important; }
  ...
  <div class="flex-none mb-2 md:mb-4 flex flex-col w-full shrink-0">
    <div class="flex items-center justify-between w-full mb-2 md:mb-3">
        <div class="flex flex-col gap-0.5 md:gap-1.5 min-w-0 flex-1">
          ...
          <p class="text-[8px] md:text-xs text-slate-500 font-medium tracking-wide ml-0 md:ml-11">* %OTP : (Total Bayar + OTP) / M-1</p>
        </div>
        ...
        <form id="formFilterRR" class="bg-white p-2 md:p-2.5 rounded-lg md:rounded-xl border border-slate-200 shadow-sm w-full" onsubmit="event.preventDefault(); fetchRekapRR();">
            <div class="flex flex-wrap items-end gap-1.5 md:gap-2 w-full">
                ...
                <div class="flex items-center gap-1 h-[30px] md:h-[38px] shrink-0 ml-auto sm:ml-0">
                    <button type="submit" id="btn-cari" class="btn-icon h-full w-[36px] md:w-[80px] bg-blue-600 hover:bg-blue-700 text-white rounded-md md:rounded-lg shadow-sm">
                    ...
                    <button type="button" onclick="exportExcelRekapRR()" class="btn-icon h-full w-[36px] md:w-[40px] bg-emerald-600 hover:bg-emerald-700 text-white rounded-md md:rounded-lg shadow-sm shrink-0">
  ...
  <div class="flex-1 min-h-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm relative flex flex-col">
    ...
    <div class="flex-1 w-full h-full overflow-auto custom-scrollbar relative">
      <table class="min-w-full text-center border-separate border-spacing-0 text-slate-700 table-fixed" id="tabelRekapRR">
        <thead class="uppercase bg-slate-50 text-slate-600 font-bold sticky top-0 z-50 text-[9px] md:text-xs tracking-wider select-none" id="headRR"></thead>
    ...
    <div class="flex flex-col bg-white border-b shrink-0 w-full z-50">
        ...
        <div id="modalFilterWrapper" class="hidden md:block w-full border-t border-slate-100 md:border-none transition-all">
            <div class="flex flex-row items-center justify-end gap-1.5 md:gap-2 px-3 py-2 md:px-4 md:pb-3 overflow-x-auto no-scrollbar">
                <select id="opt_kankas_modal" class="inp px-1 md:px-2 h-[28px] md:h-[32px] w-[90px] md:w-[140px] text-[9px] md:text-xs font-bold text-blue-800 bg-blue-50/50 border-blue-200 outline-none shrink-0 cursor-pointer" onchange="loadDetailPage(1)">
                ...
                <button onclick="downloadExcelFull()" class="btn-icon bg-emerald-600 hover:bg-emerald-700 text-white px-2 md:px-3 h-[28px] md:h-[32px] rounded-lg shadow-sm shrink-0">
...
<script>
  ...
  // AMBIL MASTER KANKAS KHUSUS UNTUK DROPDOWN MODAL (SECEPAT KILAT)
  ...
  // Populate AO dari Data Halaman Saat Ini aja biar aman
  function populateAOFromData(data, preselect) {
      ...
      // Keep existing selected value if it's not in the new page map, to avoid resetting filter
  ...
  async function fetchRekapRR(){
    ...
    try {
        ...
        const payload = { 
            ...
            kode_kantor: cabangVal || null, korwil: reqKorwil, kode_kankas: reqKankas, dpd_bucket: dpdBucket, include_127: document.getElementById('chk_127').checked
        };
        ...
        if(err.name !== 'AbortError') tb.innerHTML=`<tr><td colspan="8" class="py-16 text-center text-red-500 font-bold uppercase tracking-widest text-[10px] md:text-sm">Error: ${err.message}</td></tr>`;
          ...
          h += `
            <tr class="transition border-b border-slate-100 group h-[46px] md:h-[52px] ${bg}">
                ...
                <td class="px-2 md:px-3 py-1.5 md:py-2 border-r border-slate-100 text-center bg-blue-50/10 hover:bg-blue-50 transition">
                    <a href="javascript:void(0)" onclick="initModalDetail('${r.tgl}','ALL')" class="font-bold text-blue-700 hover:text-blue-800 hover:underline cursor-pointer block mb-0.5 text-[10px] md:text-sm">${fmt(r.target_os)}</a>
                ...
                <td class="px-2 md:px-3 py-1.5 md:py-2 border-r border-slate-100 text-center bg-green-50/10 hover:bg-green-50 transition">
                    <a href="javascript:void(0)" onclick="initModalDetail('${r.tgl}','LANCAR')" class="font-bold text-green-600 hover:text-green-700 hover:underline cursor-pointer block mb-0.5 text-[10px] md:text-sm">${fmt(r.lancar_os)}</a>
                ...
                <td class="px-2 md:px-3 py-1.5 md:py-2 border-r border-slate-100 text-center bg-red-50/10 hover:bg-red-50 transition">
                    <a href="javascript:void(0)" onclick="initModalDetail('${r.tgl}','MENUNGGAK')" class="font-bold text-red-600 hover:text-red-700 hover:underline cursor-pointer block mb-0.5 text-[10px] md:text-sm">${fmt(r.macet_os)}</a>
                ...
                <td class="px-2 md:px-3 py-1.5 md:py-2 border-r border-slate-100 text-center bg-slate-50 hover:bg-slate-100 transition">
                    <a href="javascript:void(0)" onclick="initModalLunas('${r.tgl}')" class="font-bold text-slate-700 hover:text-blue-700 hover:underline cursor-pointer block mb-0.5 text-[10px] md:text-sm">${fmt(r.lunas_os)}</a>
                ...
                <td class="px-2 md:px-3 py-1.5 md:py-2 border-r border-slate-100 text-center bg-slate-50 hover:bg-slate-100 transition align-top pt-2 md:pt-3">
                    <a href="javascript:void(0)" onclick="initModalDetail('${r.tgl}','ANGSURAN')" class="font-bold text-slate-600 hover:text-blue-700 hover:underline cursor-pointer block text-[10px] md:text-sm">${fmt(r.angsuran)}</a>
                </td>
                <td class="px-2 md:px-3 py-1.5 md:py-2 text-center bg-purple-50/10 hover:bg-purple-50 transition align-top pt-2 md:pt-3 border-r border-slate-100">
                    <a href="javascript:void(0)" onclick="initModalDetail('${r.tgl}','TOTAL_BAYAR')" class="font-bold text-purple-700 hover:text-purple-800 hover:underline cursor-pointer block text-[10px] md:text-sm">${fmt(r.total_bayar)}</a>
                </td>
  ...
  function renderModalHeaderRR() {
      ...
      if (currentMode === 'NORMAL') {
          mHead.innerHTML = `
              <tr>
                  <th class="col-rek hidden md:table-cell px-2 md:px-3 border-b border-r border-slate-300 rounded-tl-lg text-left md:text-center cursor-pointer hover:bg-slate-200 transition select-none" onclick="sortDetailRR('no_rekening', 'string')">
                  ...
                  <th class="col-nas px-2 md:px-4 border-b border-r border-slate-300 text-left md:text-center cursor-pointer hover:bg-slate-200 transition select-none" onclick="sortDetailRR('nama_nasabah', 'string')">
          ...
          mHead.innerHTML = `
              <tr>
                  <th class="col-nas-lunas px-2 md:px-4 border-b border-r border-slate-300 rounded-tl-lg text-left md:text-center cursor-pointer hover:bg-slate-200 transition select-none" onclick="sortDetailRR('nama_nasabah', 'string')">
                      <div class="flex items-center justify-start md:justify-center">NAMA NASABAH ${getSortIcon('nama_nasabah', sortDetailCol, sortDetailAsc)}</div>
                  </th>
                  <th class="px-2 md:px-4 border-b border-r border-slate-300 w-[200px] md:w-[350px] text-left md:text-center cursor-pointer hover:bg-slate-200 transition select-none" onclick="sortDetailRR('alamat', 'string')">
                      <div class="flex items-center justify-start md:justify-center">ALAMAT ${getSortIcon('alamat', sortDetailCol, sortDetailAsc)}</div>
  ...
  async function initModalDetail(tgl, status) {
      ...
      currentDetailParams = { 
          ...
          kode_kantor: branch, korwil: reqKorwil, kode_kankas: preselectKankasCode || reqKankas, kode_ao: "",
      ...
      // Panggil Master Kankas biar loading dropdown kilat
  ...
  async function loadDetailPage(page) {
      ...
      try {
          ...
          // Populate AO dropdown dari data yang sedang aktif (biar ga berat fetch semua AO)
  ...
  function renderTableDetailBodyRR(list) {
      ...
      list.forEach(r => {
          ...
          if(currentMode === 'NORMAL') {
              ...
              const kodeKankas = r.kode_kankas || r.kode_group1 || r.kankas;

              h += `<tr class="transition border-b border-slate-100 h-[40px] md:h-[48px]">
                    <td class="col-rek hidden md:table-cell px-2 md:px-3 py-1.5 md:py-2 border-r border-slate-100 font-mono text-[9.5px] md:text-sm text-slate-600">${r.no_rekening}</td>
                    <td class="col-nas px-2 md:px-4 py-1.5 md:py-2 border-r border-slate-100 font-bold text-slate-700 truncate text-[9.5px] md:text-sm" title="${r.nama_nasabah}">${r.nama_nasabah}</td>
                    ...
                    <td class="px-2 md:px-3 py-1.5 md:py-2 border-r border-slate-100 text-center font-mono text-[9px] md:text-sm">
                        ${r.kankas && r.kankas !== '-' ? `<a href="javascript:void(0)" onclick="setKankasModal('${kodeKankas}')" class="text-blue-600 hover:text-blue-800 hover:underline cursor-pointer font-bold transition-all" title="Klik untuk filter Kankas ini">${r.kankas}</a>` : '-'}
                    </td>
                    <td class="px-2 md:px-4 py-1.5 md:py-2 border-r border-slate-100 text-center font-bold text-[9.5px] md:text-sm text-blue-700 truncate">
                        ${r.kode_ao || r.nama_ao ? `<a href="javascript:void(0)" onclick="setAOModal('${r.kode_ao || r.nama_ao}')" class="hover:underline hover:text-blue-900 cursor-pointer" title="Klik untuk filter AO ini">${aoName}</a>` : aoName}
                    </td>
              ...
              h += `<tr class="transition border-b border-slate-100 h-[40px] md:h-[48px]">
                    <td class="col-nas-lunas px-2 md:px-4 py-1.5 md:py-2 border-r border-slate-100 font-bold text-slate-700 truncate text-[9.5px] md:text-sm">
                        ${r.nama_nasabah}
