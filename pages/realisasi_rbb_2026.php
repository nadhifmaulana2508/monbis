...
$rrbRenderMetricCard = static function (array $card, callable $escape, bool $shortNominal = false) use ($rrbIconSvg, $rrbShortNominal): string {
    ...
    $metricValue = static function ($value) use ($escape, $rrbShortNominal, $isRatio, $shortNominal): string {
        ...
        $display = !$isRatio && $shortNominal ? $rrbShortNominal($full) : $full;
        $title = !$isRatio && $shortNominal ? ' title="' . $escape('Angka asli: Rp ' . $full) . '"' : '';
        return '<strong class="rrb26-number-tooltip"' . $title . '>' . $escape($display) . '</strong>';
    };
    $footer = $shortNominal
        ? 'Dalam jutaan rupiah · arahkan kursor untuk angka asli'
        : 'Dalam rupiah';
