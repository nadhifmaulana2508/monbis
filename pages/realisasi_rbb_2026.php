...
$rrbRenderMetricCard = static function (array $card, callable $escape) use ($rrbIconSvg, $rrbShortNominal): string {
    ...
    $metricValue = static function ($value) use ($escape, $rrbShortNominal, $isRatio): string {
        $full = trim((string) $value);
        $display = $isRatio ? $full : $rrbShortNominal($full);
        $title = $isRatio ? '' : ' title="' . $escape('Angka asli: Rp ' . $full) . '"';
        return '<strong class="rrb26-number-tooltip"' . $title . '>' . $escape($display) . '</strong>';
    };
    return '<article class="rrb26-card rrb26-tone-' . $escape($card['tone']) . $ratioClass . '">' .
        ...
        '<div class="rrb26-values"><div><small>RBB Agustus 2026</small>' . $metricValue($card['rbb']) . '</div><div><small>Realisasi Agustus</small>' . str_replace('rrb26-number-tooltip', 'rrb26-number-tooltip rrb26-actual', $metricValue($card['actual'])) . '</div><div><small>RBB Des 2026</small>' . $metricValue($card['year']) . '</div></div>' .
        ...
        '<footer>Dalam jutaan rupiah · arahkan kursor untuk angka asli</footer></article>';
};
?>
