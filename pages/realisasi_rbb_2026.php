...
$rrbRenderMetricCard = static function (array $card, callable $escape): string {
    ...
    return '<article class="rrb26-card rrb26-tone-' . $escape($card['tone']) . $ratioClass . '">' .
        '<div class="rrb26-card-head"><div class="rrb26-card-icon">' . $escape($card['icon']) . '</div><div><h3>' . $escape($card['name']) . '</h3>' . $caption . '</div></div>' .
        '<div class="rrb26-values"><div><small>RBB Agustus 2026</small><strong>' . $escape($card['rbb']) . '</strong></div><div><small>Realisasi Agustus</small><strong class="rrb26-actual">' . $escape($card['actual']) . '</strong></div><div><small>RBB Des 2026</small><strong>' . $escape($card['year']) . '</strong></div></div>' .
        '<div class="rrb26-achievements"><div><small>% thd RBB Ags</small><b class="' . $periodClass . '">' . $escape($card['period']) . '</b></div><div><small>% thd Des</small><b class="' . $yearClass . '">' . $escape($card['year_pct']) . '</b></div></div>' .
        '<footer>Dalam Rupiah</footer></article>';
};
?>
...
<div id="realisasiRbb2026" class="rrb26-page">
  <section class="rrb26-header">
    <div class="rrb26-heading">
      ...
      <div>
        ...
        <p>Snapshot capaian Agustus 2026 dari materi laporan yang diunggah.</p>
      </div>
    </div>
    <div class="rrb26-header-meta"><span class="rrb26-live-dot"></span><span>Data sementara</span><small>Update manual · Agustus 2026</small></div>
  </section>
  ...
  <section class="rrb26-workspace">
    ...
    <div class="rrb26-note"><span><b>Posisi:</b> Agustus 2026</span><span><b>Satuan:</b> ribuan rupiah</span><span class="rrb26-note-warning">Snapshot manual · menunggu penyamaan data mentah</span></div>
    ...
    <section id="rrb26-panel-kinerja_pusat" class="rrb26-panel<?= $rrbView === 'kinerja_pusat' ? ' active' : '' ?>" role="tabpanel" data-rrb26-panel="kinerja_pusat"<?= $rrbView === 'kinerja_pusat' ? '' : ' hidden' ?> >
      ...
      <div class="rrb26-card-grid rrb26-central-grid"><?php foreach ($rrbCentralCards as $card): echo $rrbRenderMetricCard($card, $rrbEscape); endforeach; ?></div>
    </section>
    ...
    <?php foreach ($rrbRegionalCards as $key => $region): ?>
      <section id="rrb26-panel-<?= $rrbEscape($key) ?>" class="rrb26-panel<?= $rrbView === $key ? ' active' : '' ?>" role="tabpanel" data-rrb26-panel="<?= $rrbEscape($key) ?>"<?= $rrbView === $key ? '' : ' hidden' ?> >
        ...
        <div class="rrb26-card-grid rrb26-regional-grid"><?php foreach ($region['cards'] as $card): echo $rrbRenderMetricCard($card, $rrbEscape); endforeach; ?></div>
      </section>
    <?php endforeach; ?>
