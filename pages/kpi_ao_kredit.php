...
function headers(extra={}){let token='';try{token=String(window.AUTH_TOKEN||localStorage.getItem('dpk_token')||'').trim()}catch(error){}if(!token){const match=document.cookie.match(/(?:^|;\s*)sso_token=([^;]+)/);if(match)try{token=decodeURIComponent(match[1])}catch(error){token=match[1]}}const result=Object.assign({'Content-Type':'application/json'},extra);if(token)result.Authorization=/^Bearer\s/i.test(token)?token:`Bearer ${token}`;return result}
