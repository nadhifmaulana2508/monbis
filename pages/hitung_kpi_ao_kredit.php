...
<script>
window.MonbisKpiHeaders=window.MonbisKpiHeaders||function(){const match=document.cookie.match(/(?:^|;\s*)sso_token=([^;]+)/),token=String((match?decodeURIComponent(match[1]):'')||window.AUTH_TOKEN||localStorage.getItem('dpk_token')||'').replace(/^Bearer\s+/i,'').trim(),headers={'Content-Type':'application/json'};if(token)headers.Authorization='Bearer '+token;return headers};
