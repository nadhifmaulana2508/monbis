...
$hitungKpiRunTone = $hitungKpiDefaultMode === 'bulk' ? 'primary' : 'success';
$hitungKpiRunIcon = $hitungKpiDefaultMode === 'bulk' ? 'zap' : 'chart';
mb_ui_assets('.');
...
mb_render_report_page([
    ...
    'header'=>[
        ...
        'filters'=>[
            ...
            ['id'=>'hitungKpiClosing','label'=>'Closing','type'=>'select','width'=>'130px','field_class'=>'hitung-kpi-closing-filter','options'=>[''=>'Memuat...']],
    ...
    'toolbar'=>[
        ...
        'actions'=>[['attrs'=>['id'=>'hitungKpiRun'],'tone'=>$hitungKpiRunTone,'icon'=>$hitungKpiRunIcon,'label'=>$hitungKpiDefaultMode === 'bulk' ? '' : 'Hitung','title'=>'Hitung AO terpilih','aria_label'=>'Hitung AO terpilih']],
    ],
...
.mb-kpi-calc-page #hitungKpiRun.mb-kpi-run--labeled{width:70px;min-width:70px;padding:0 8px}
...
(()=>{
  ...
  const state={boot:null,detail:[],evaluation:null,mode:'single'};let refreshBulkRows=()=>{};
  const job=()=>el('hitungKpiJabatan')?.value||'AO_KREDIT';
  let aoList=()=>state.boot?.ao||[];
  ...
  function fill(d,keepOffice=''){
    ...
    state.detail=[];state.evaluation=null;el('hitungKpiFinal').textContent='-';el('hitungKpiStatus').textContent='BELUM DIHITUNG';el('hitungKpiNote').textContent='Pilih jabatan, kantor, AO, dan closing';if(keepOffice)fillAo();else render();refreshBulkRows();
  }
  ...
  async function loadSavedDetail(){const ao=currentAo(),closing=el('hitungKpiClosing')?.value;if(!ao||!closing){state.detail=[];state.evaluation=null;render();return}const saved=existingFor(ao,closing);if(!saved){state.detail=[];state.evaluation=null;render();return}try{const result=await post({type:'evaluation',year:el('hitungKpiYear').value,jabatan_kode:job(),kode_ao:ao.kode_ao,id_peg:ao.id_peg,kode_kantor:ao.kode_kantor,closing_date:closing}),row=result.data?.[0];state.evaluation=row||null;state.detail=row?(await post({type:'detail',penilaian_id:row.id})).data||[]:[];render()}catch(error){state.detail=[];state.evaluation=null;render()}}
  function render(){
    ...
    const saved=existingFor(ao,el('hitungKpiClosing')?.value);if(state.evaluation){el('hitungKpiFinal').textContent=fmt(state.evaluation.nilai_akhir);el('hitungKpiStatus').textContent=state.evaluation.status||'SUDAH DIGENERATE';el('hitungKpiNote').textContent=state.evaluation.predikat||'Nilai tersimpan';}else if(!rows.length){el('hitungKpiFinal').textContent=saved?fmt(saved.nilai_akhir):'-';el('hitungKpiStatus').textContent=saved?(saved.status||'SUDAH DIGENERATE'):'BELUM DIHITUNG';el('hitungKpiNote').textContent=saved?'Nilai tersimpan. Breakdown dimuat otomatis.':'Pilih jabatan, kantor, AO, dan closing';}
  ...
  async function run(){
    ...
    const btn=el('hitungKpiRun');state.evaluation=null;btn.disabled=true;ui().showLoading?.('hitungKpiLoading',true);
  ...
  el('hitungKpiJabatan')?.addEventListener('change',boot);el('hitungKpiYear')?.addEventListener('change',boot);el('hitungKpiKantor')?.addEventListener('change',async()=>{const branch=el('hitungKpiKantor').value;if(!branch){state.boot.ao=[];fillAo();return}try{fill(await post({type:'bootstrap',year:el('hitungKpiYear').value||new Date().getFullYear(),jabatan_kode:job(),kode_kantor:branch}),branch)}catch(e){alert(e.message)}});el('hitungKpiAo')?.addEventListener('change',()=>{state.detail=[];state.evaluation=null;loadSavedDetail()});el('hitungKpiClosing')?.addEventListener('change',()=>{state.detail=[];state.evaluation=null;loadSavedDetail()});el('hitungKpiRun')?.addEventListener('click',run);el('hitungKpiSearch')?.addEventListener('input',e=>{const q=e.target.value.toLowerCase();document.querySelectorAll('#hitungKpiBody tr').forEach(r=>r.style.display=r.textContent.toLowerCase().includes(q)?'':'none')});
  ...
  if(runButton&&tools&&page){
    ...
    const single=[page.querySelector('.mb-summary'),page.querySelector('.mb-table-region')].filter(Boolean);const activate=mode=>{state.mode=mode;single.forEach(x=>x.classList.toggle('is-hidden',mode!=='single'));panel.classList.toggle('is-hidden',mode!=='bulk');el('hitungKpiAoTitle').textContent=mode==='single'?'Detail Penilaian Bulanan':'Generate KPI per Cabang';runButton.title=mode==='single'?'Hitung AO terpilih':'Generate KPI per cabang';runButton.setAttribute('aria-label',runButton.title);runButton.classList.toggle('mb-icon-button--primary',mode==='bulk');runButton.classList.toggle('mb-icon-button--success',mode!=='bulk');runButton.innerHTML=mode==='single'?'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/></svg>':'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13 2 4 14h7l-1 8 9-12h-7l1-8Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';switcher.title=mode==='single'?'Buka Generate Cabang':'Kembali ke Nilai AO';switcher.setAttribute('aria-label',switcher.title)};activate(<?= json_encode($hitungKpiDefaultMode) ?>==='bulk'||new URLSearchParams(location.search).get('mode')==='bulk'?'bulk':'single');switcher.addEventListener('click',()=>activate(state.mode==='single'?'bulk':'single'));
  ...
  refreshBulkRows=()=>{const body=el('hitungKpiBulkBody');if(!body||state.mode!=='bulk')return;const branch=el('hitungKpiKantor')?.value||'',aos=aoList().filter(x=>!branch||String(x.kode_kantor)===String(branch)),generated=(state.boot?.generated||[]).filter(x=>!branch||String(x.kode_kantor)===String(branch));if(!generated.length){body.innerHTML='<tr><td colspan="5" class="mb-empty">Belum ada penilaian yang digenerate untuk kantor ini.</td></tr>';return}body.innerHTML=generated.map(item=>{const person=aos.find(x=>String(x.id_peg||x.kode_ao)===String(item.id_peg||item.kode_ao)||String(x.kode_ao)===String(item.kode_ao));return `<tr><td>${esc(person?.nama_ao||item.nama_ao||item.kode_ao||'-')}</td><td>${esc(item.kode_kantor||branch||'-')}</td><td>${esc(item.closing_date||'-')}</td><td>${esc(item.status||'SUDAH DIGENERATE')}</td><td>${fmt(item.nilai_akhir)}</td></tr>`}).join('')};
  const closingFilter=el('hitungKpiClosing')?.closest('.mb-field'),syncClosingFilter=()=>closingFilter?.classList.toggle('is-hidden',state.mode==='bulk');syncClosingFilter();document.querySelector('#hitungKpiAoPage .mb-view-switch')?.addEventListener('click',()=>setTimeout(syncClosingFilter,0));
  // Mode generate hanya memproses AO yang dipilih; tanpa pilihan tetap seluruh kantor.
  const filteredAoList=()=>{const list=state.boot?.ao||[],selected=el('hitungKpiAo')?.value||'';return state.mode==='bulk'&&selected?list.filter(x=>String(x.kode_ao)===String(selected)):list};
  const selectedBulkRows=()=>{const body=el('hitungKpiBulkBody');if(!body||state.mode!=='bulk')return;const branch=el('hitungKpiKantor')?.value||'',selected=el('hitungKpiAo')?.value||'',aos=filteredAoList(),generated=(state.boot?.generated||[]).filter(x=>(!branch||String(x.kode_kantor)===String(branch))&&(!selected||String(x.id_peg||x.kode_ao)===String(selected)));if(!generated.length){body.innerHTML='<tr><td colspan="5" class="mb-empty">Belum ada penilaian yang digenerate untuk pilihan ini.</td></tr>';return}body.innerHTML=generated.map(item=>{const person=aos.find(x=>String(x.id_peg||x.kode_ao)===String(item.id_peg||item.kode_ao)||String(x.kode_ao)===String(item.kode_ao));return `<tr><td>${esc(person?.nama_ao||item.nama_ao||item.kode_ao||'-')}</td><td>${esc(item.kode_kantor||branch||'-')}</td><td>${esc(item.closing_date||'-')}</td><td>${esc(item.status||'SUDAH DIGENERATE')}</td><td>${fmt(item.nilai_akhir)}</td></tr>`}).join('')};
  aoList=filteredAoList;refreshBulkRows=selectedBulkRows;el('hitungKpiAo')?.addEventListener('change',()=>setTimeout(refreshBulkRows,0));
  const selectedBulkRowsFixed=()=>{const body=el('hitungKpiBulkBody');if(!body||state.mode!=='bulk')return;const branch=el('hitungKpiKantor')?.value||'',selected=el('hitungKpiAo')?.value||'',people=state.boot?.ao||[],person=people.find(x=>String(x.kode_ao)===String(selected)),aos=filteredAoList(),generated=(state.boot?.generated||[]).filter(x=>{if(branch&&String(x.kode_kantor)!==String(branch))return false;if(!selected)return true;return String(x.kode_ao||'')===String(selected)||String(x.id_peg||'')===String(selected)||!!person&&String(x.id_peg||'')===String(person.id_peg||'')});if(!generated.length){body.innerHTML='<tr><td colspan="5" class="mb-empty">Belum ada penilaian yang digenerate untuk pilihan ini.</td></tr>';return}body.innerHTML=generated.map(item=>{const matched=aos.find(x=>String(x.id_peg||x.kode_ao)===String(item.id_peg||item.kode_ao)||String(x.kode_ao)===String(item.kode_ao)||!!person&&String(x.id_peg||'')===String(item.id_peg||''));return `<tr><td>${esc(matched?.nama_ao||item.nama_ao||item.kode_ao||'-')}</td><td>${esc(item.kode_kantor||branch||'-')}</td><td>${esc(item.closing_date||'-')}</td><td>${esc(item.status||'SUDAH DIGENERATE')}</td><td>${fmt(item.nilai_akhir)}</td></tr>`}).join('')};refreshBulkRows=selectedBulkRowsFixed;
  const syncRunButtonLabel=()=>{const button=el('hitungKpiRun');if(!button)return;let label=button.querySelector('.mb-icon-button__label');button.classList.toggle('mb-kpi-run--labeled',state.mode==='single');if(state.mode==='single'){if(!label){label=document.createElement('span');label.className='mb-icon-button__label';button.appendChild(label)}label.textContent='Hitung'}else if(label)label.remove()};syncRunButtonLabel();document.querySelector('#hitungKpiAoPage .mb-view-switch')?.addEventListener('click',()=>setTimeout(syncRunButtonLabel,0));
  document.querySelector('#hitungKpiAoPage')?.addEventListener('click',event=>{if(event.target.closest('.mb-view-switch'))setTimeout(refreshBulkRows,0)});
