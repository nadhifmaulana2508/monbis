...
$head = mb_build_grouped_thead([
    ...
    ['label'=>'KETERANGAN','attrs'=>['style'=>'width:220px']],
]);
...
.mb-kpi-calc-page #hitungKpiTable{min-width:980px}
...
@media(max-width:700px){.mb-kpi-calc-page #hitungKpiTable{min-width:820px}.mb-kpi-calc-page .mb-kpi-bulk{padding:8px}.mb-kpi-calc-page .mb-kpi-bulk__table-wrap{max-height:300px}}
</style>
<script>
...
(()=>{
  ...
  function showValue(x){return x.unit==='RUPIAH'?money(x):x.unit==='PERSEN'?pct(x):fmt(x)}
  function existingFor(ao,closing){return (state.boot?.generated||[]).find(x=>String(x.id_peg||x.kode_ao)===String(ao?.id_peg||ao?.kode_ao)&&String(x.closing_date)===String(closing))||null}
  function render(){
    ...
    body.innerHTML=rows.length?rows.map(x=>`<tr><td class="mb-sticky-left mb-text-left"><strong>${esc(x.nama)}</strong><small class="mb-subvalue">${esc(x.kelompok||'')}</small></td><td class="mb-num">${showValue(x.target)}</td><td class="mb-num mb-strong">${showValue(x.realisasi)}</td><td class="mb-num">${x.unit==='PERSEN'?pct(x.indeks):fmt(x.indeks)}</td><td class="mb-num">${fmt(x.skor)} / 5</td><td class="mb-num">${pct(x.bobot)}</td><td class="mb-num mb-strong">${fmt(x.nilai_100)}</td><td>${esc(x.catatan||'Terhitung')}</td></tr>`).join(''):'<tr><td colspan="8" class="mb-empty">Pilih jabatan, kantor, AO, dan closing, lalu klik Hitung KPI.</td></tr>';
  }
  async function boot(){try{fill(await post({type:'bootstrap',year:el('hitungKpiYear').value||new Date().getFullYear(),jabatan_kode:job()}));}catch(e){el('hitungKpiBody').innerHTML=`<tr><td colspan="8" class="mb-empty mb-negative">${esc(e.message)}</td></tr>`}}
