...
function v2_render_sidebar(string $active, string $baseUrl, string $legacyBase = ''): void
{
    ...
    $moduleLink = static fn(string $page, string $tab): string => $link($page) . '/?tab=' . rawurlencode($tab);
    ...
    echo '<a class="v2-nav-sub-item" href="' . v2_e($moduleLink('rbb', 'detail') . '&category=KREDIT') . '" title="Input RBB Kredit">Input RBB Kredit</a>';
    echo '<a class="v2-nav-sub-item" href="' . v2_e($moduleLink('rbb', 'detail') . '&category=DAMAS') . '" title="Input RBB DAMAS">Input RBB DAMAS</a>';
    echo '<a class="v2-nav-sub-item" href="' . v2_e($moduleLink('rbb', 'detail') . '&category=PENDAPATAN') . '" title="Input RBB Pendapatan">Input RBB Pendapatan</a>';
    echo '<a class="v2-nav-sub-item" href="' . v2_e($moduleLink('rbb', 'detail') . '&category=BEBAN') . '" title="Input RBB Beban">Input RBB Beban</a>';
    echo '</div></div>';
