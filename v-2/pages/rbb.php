...
(() => {
  ...
  async function init() { try { const response = await fetch(API_KODE, {method:'POST', credentials:'include', headers:authHeaders(), body:JSON.stringify({type:'kode_kantor'})}); const json = await response.json(); fillOffices(Array.isArray(json.data) ? json.data.filter((item) => String(item.kode_kantor) !== '000') : []); await load(); } catch (error) { const target = TAB === 'projection' ? 'v2RbbProjectionBody' : TAB === 'detail' ? 'v2RbbDetailBody' : 'v2RbbAbaBody'; showError(target, error.message, 6); } }
