...
<?php if ($tab !== 'aba'): ?>
    <div class="v2-rbb-header-controls">
      ...
      <form class="v2-rbb-print-link-form" id="v2RbbPrintForm" method="post" action="<?= v2_e($baseUrl . '/rbb/print') ?>"><input type="hidden" name="kode_kantor" id="v2RbbPrintBranchInput"><input type="hidden" name="tahun_mulai" id="v2RbbPrintYearInput"><input type="hidden" name="bulan_mulai" id="v2RbbPrintMonthInput"><button type="submit" class="v2-button v2-button--soft v2-rbb-print-link" id="v2RbbPrint"><?= v2_icon('printer', 14) ?><span>Cetak</span></button></form>
    </div>
<?php endif; ?>
  </div>
...
(() => {
  ...
  const API_LAPKEU = <?= json_encode($legacyBase . '/api/lapkeu/') ?>;
  const API_KODE = <?= json_encode($legacyBase . '/api/kode/') ?>;
  ...
  const postTo = async (url, body, label = 'RBB') => { const response = await fetch(url, {method:'POST', credentials:'same-origin', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)}); const json = await response.json().catch(() => ({})); if (!response.ok || Number(json.status) !== 200) throw new Error(json.message || `Request ${label} gagal (${response.status})`); return json.data || {}; };
